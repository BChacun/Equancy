...
def app(df):
    ...
    ep_prec = None
    ep = st.number_input('Choose the number of epochs', min_value=10, max_value=100000, value=100, step=100)
    if ep_prec != ep:
        ep_prec = ep
        change = True
        
    l_prec = None
    l = st.number_input('Choose learning_rate', min_value=0.001, max_value=0.1, value=0.01, step=0.002)
    if l_prec != l:
        l_prec = l
        change = True
