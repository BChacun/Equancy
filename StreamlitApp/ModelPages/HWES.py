...
def initStates(S, df):
    if 'results_HWES' not in st.session_state:
        st.session_state.resultsHWES = HWES(S, df)
    if 'state_S' not in st.session_state:
...
def inputParameters():
    ...
    S = container.number_input('Choose S', min_value=1, max_value=50, value=49, step=1)
    return S
...
def app(df):
    ...
    initStates(S, df)
    changeStates(S, df)
    results, mean_error, mae, rmse, mape, mapd, duree = st.session_state.resultsHWES
